...
AI에 Playwright MCP 같은 브라우저 도구를 연결하면 웹 자동화가 가능하지만, 두 가지 근본적인 문제가 있습니다.

**1. 토큰 낭비** — 로그인, VPN 신청, Jira 티켓 생성처럼 매일 똑같이 반복하는 작업에도 매번 AI가 페이지를 해석하고 판단하느라 토큰을 소비합니다. 결과가 동일한 작업에 매번 비용을 쓰는 것은 비효율적입니다.

**2. 보안 위험** — AI에게 OTP나 비밀번호를 맡기면 민감 정보가 AI 컨텍스트에 노출됩니다. OTP Secret Key를 AI가 읽고, 외부 API로 전송하는 구조 자체가 보안 취약점이 됩니다.

RecordFlow는 이 두 가지를 모두 해결합니다:

| | AI 브라우저 자동화 (MCP) | RecordFlow |
|---|---|---|
| **녹화** | AI가 페이지를 해석하며 토큰 소비 | Playwright Codegen이 조작을 기록 (토큰 0) |
| **재생** | 매 실행마다 AI 추론 + 토큰 소비 | 저장된 스텝을 그대로 재생 (토큰 0) |
| **스케줄** | 실행할 때마다 AI 호출 필요 | Cron/예약으로 무인 실행 (토큰 0) |
| **OTP/비밀번호** | AI 컨텍스트에 민감정보 노출 | 로컬 OS 암호화 저장, AI 경유 없음 |

> **한 번 녹화하면 이후 실행에 토큰이 전혀 들지 않고, 민감 정보는 로컬에서만 처리됩니다.**
>
> AI는 워크플로우를 *설계할 때* 활용하고, 반복 실행과 인증은 RecordFlow에 맡기세요.
...
- **재녹화** - 기존 워크플로우에 새로운 조작을 추가 녹화
- **폴더 관리** - 워크플로우를 폴더 트리 구조로 정리
- **워크플로우 공유** - `.rfworkflow` 파일로 내보내기/가져오기 (민감정보 자동 마스킹)

### 스케줄링
- **Cron / 일회성 예약** - Cron 표현식 또는 특정 시각으로 워크플로우 자동 실행
- **스케줄 폴더** - 담당자별·용도별로 스케줄을 폴더 트리로 관리
- **개인화 (독립 Step 복사본)** - 스케줄 생성 시 워크플로우 Step을 독립 복사하여 계정·비밀번호·OTP 등을 개인별로 설정 가능 (원본 워크플로우 템플릿 미변경)
- **스케줄 수정** - 생성된 스케줄의 Cron/시각, 연결 워크플로우를 변경 가능
- **즉시 실행** - 스케줄을 수동으로 바로 실행하여 결과 확인
...
│   │   ├── schedule/      # SchedulePanel, ScheduleFolderItem, ScheduleItem, ScheduleDetail, ScheduleDialog
│   │   ├── settings/      # SettingsPanel, OtpSection
│   │   ├── dialogs/       # EditValueDialog, DateVariableHelper, ImportWorkflowDialog, ConfirmDialog, ReRecordDialog 등
│   │   └── ui/            # Input

...
모든 데이터는 **로컬 JSON 파일**로만 저장되며 외부 서버로 전송되지 않습니다.

| 데이터 | 저장 위치 | 내용 |
|--------|-----------|------|
| 워크플로우 (아이디, 비밀번호 등) | `%APPDATA%/recordflow/workflows.json` | 스텝에 녹화된 값이 평문으로 저장 |
| OTP Secret Key | `%APPDATA%/recordflow/settings.json` | Base32 키가 평문으로 저장 |

> **참고**: 현재 버전에서는 로컬 파일을 암호화하지 않습니다. 사용자 PC의 OS 계정 보호(로그인 잠금 등)에 의존합니다.
